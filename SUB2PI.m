...
H1=[];H2=[];H3=[];
 ...
 noise1= a1*0.01*(sin(1*i)^2*cos(9*i)+sin(2*i)^2*cos(0.1*i)+ ...
     sin(-1.2*i)^2*cos(0.5*i)+s ...
     in(i)^5+sin(1.12*i)^2+cos(2.4*i)*sin(2.4*i)^3);
ns(i)=noise1;
    u=K*x(:,i)+noise1;   
    x(:,i+1)=A1*x(:,i)+B1*u;
   ...
   zv1=[x(1,i)^2;x(1,i)*x(2,i);x(1,i)*u;x(2,i)^2;x(2,i)*u;u^2];
   uk=K*x( :,i+1);
   zv2=[x(1,i+1)^2;x(1,i+1)*x(2,i+1);...
       x(1,i+1)*uk;x(2,i+1)^2;x(2,i+1)*uk;uk^2];
    zbar(:,6)=zv1-zv2;
    if mod(i,6)==0
       if i<=40
        m1=zbar*zbar';
